...
import { BookOpen, ChevronLeft, ChevronRight, LogOut, Home, CreditCard, Calendar, FileText, Award, Users, ClipboardList, Settings, BarChart3, Moon } from "lucide-react";
...
const menuConfig: Record<string, { icon: any; label: string }[]> = {
  mahasiswa: [
    { icon: Home, label: "Dashboard" },
    { icon: CreditCard, label: "Pembayaran" },
    { icon: Calendar, label: "Jadwal Saya" },
    { icon: FileText, label: "Hasil Mengaji" },
    { icon: Award, label: "Sertifikat" },
  ],
  instruktur: [
    { icon: Home, label: "Dashboard" },
    { icon: Users, label: "Daftar Mahasiswa" },
    { icon: ClipboardList, label: "Penilaian" },
  ],
  admin: [
    { icon: Home, label: "Dashboard" },
    { icon: Users, label: "Mahasiswa" },
    { icon: Users, label: "Instruktur" },
    { icon: CreditCard, label: "Pembayaran" },
    { icon: Calendar, label: "Jadwal" },
    { icon: ClipboardList, label: "Penilaian & Hasil" },
    { icon: Award, label: "Sertifikat" },
    ...
    <div className="flex min-h-screen" style={{ background: "#FAF7F0" }}>
      ...
      <main className="flex-1 overflow-auto">
        <header className="h-16 flex items-center justify-between px-8 border-b" style={{ borderColor: "#e8e4db", background: "rgba(250, 247, 240, 0.8)", backdropFilter: "blur(8px)" }}>
          <h2 className="text-lg font-semibold" style={{ color: "#1A1A1A" }}>{currentPage}</h2>
