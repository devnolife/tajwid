...
import { Calendar, MapPin, Clock, ClipboardList, CheckCircle2, History, ChevronRight, UserX, RotateCcw, Plus, Pencil } from "lucide-react";
import type { Schedule, User as UserType, Assessment } from "@shared/schema";
import { getMahasiswaPhotoUrl } from "@/lib/mahasiswa-photo";
import { createInstructorSchedule, updateInstructorSchedule } from "@/lib/schedule-client";
...
export default function JadwalUjianMengaji() {
  ...
  const [scheduleDialog, setScheduleDialog] = useState<{ mode: "create" | "edit"; scheduleId?: string } | null>(null);
  ...
  const openEditSchedule = (schedule: Schedule) => {
    setScheduleForm({
      ...
      date: toLocalDateTime(new Date(schedule.date)),
      room: schedule.room,
      location: schedule.location ?? "",
    });
    setScheduleDialog({ mode: "edit", scheduleId: schedule.id });
  };
  ...
  const saveSchedule = useMutation({
    mutationFn: async () => {
      if (!user?.id || !scheduleDialog) throw new Error("Sesi tidak valid");
      if (!scheduleForm.studentId || !scheduleForm.date || !scheduleForm.room.trim()) {
        throw new Error("Mahasiswa, tanggal, dan ruang wajib diisi");
      }
      if (scheduleDialog.mode === "edit" && scheduleDialog.scheduleId) {
        return updateInstructorSchedule<Schedule>(scheduleDialog.scheduleId, {
          date: scheduleForm.date,
          room: scheduleForm.room.trim(),
          location: scheduleForm.location.trim() || null,
        });
      }
      return createInstructorSchedule<Schedule>({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Berhasil", description: scheduleDialog?.mode === "edit" ? "Jadwal diperbarui" : "Jadwal baru dibuat" });
  ...
  const isAssessed = (scheduleId: string) => assessments?.some((assessment) => assessment.scheduleId === scheduleId);
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-3 text-right">
            ...
            <Button
              ...
              style={{ background: C.gold, color: C.emeraldDeep }}
            >
        ...
        <section key={dateLabel}>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {items.map((s, idx) => {
              ...
              const done = s.status === "completed" || isAssessed(s.id);
              const noShow = s.status === "no_show";
              const cancelled = s.status === "cancelled";
              const isRepeat = (s as any).isRepeat;
              const disabled = cancelled || done;
                  ...
                  {!done && !noShow && !cancelled && (
                    <div className="mt-3 pt-3 border-t flex items-center justify-between gap-2" style={{ borderColor: `${C.taupe}88` }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          type="button"
                          onClick={() => openEditSchedule(s as Schedule)}
                          className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full border transition-colors hover:bg-emerald-50"
                          style={{ color: C.emerald, borderColor: `${C.emerald}55` }}
                        >
        ...
        <DialogContent className="rounded-2xl" style={{ background: C.cream, borderColor: C.taupe }}>
          <DialogHeader>
            <DialogTitle className="font-display italic text-2xl" style={{ color: C.emerald }}>
              {scheduleDialog?.mode === "edit" ? "Edit Jadwal Tes" : "Buat Jadwal Tes"}
            </DialogTitle>
            <DialogDescription>
              Jadwal baru otomatis ditugaskan kepada akun instruktur Anda.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="schedule-student">Mahasiswa</Label>
              <select
                id="schedule-student"
                value={scheduleForm.studentId}
                onChange={(event) => setScheduleForm((current) => ({ ...current, studentId: event.target.value }))}
                disabled={scheduleDialog?.mode === "edit"}
                className="w-full h-11 rounded-xl border px-3 text-sm bg-white disabled:opacity-70"
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setScheduleDialog(null)} className="rounded-xl">
              Batal
            </Button>
            <Button
              onClick={() => saveSchedule.mutate()}
              disabled={saveSchedule.isPending || !scheduleForm.studentId || !scheduleForm.date || !scheduleForm.room.trim()}
              className="rounded-xl"
              style={{ background: C.emerald, color: C.cream }}
            >
              {saveSchedule.isPending ? "Menyimpan..." : "Simpan Jadwal"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
